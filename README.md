...
Hệ thống hỗ trợ tự động hóa việc tạo bài tập thông qua API của các LLM (OpenAI, Claude).

### 1. Cách dùng trên giao diện Web
- **Cấu hình:** Nhập **API Key**, chọn **Provider** (OpenAI/Claude) và **Model** (ví dụ: `gpt-4o`) trên thanh công cụ, sau đó nhấn **Save Config**.
- **Sinh đề:** Nhấn nút **"+ Generate New"** $\rightarrow$ AI sẽ tự động soạn đề, sinh testcase $\rightarrow$ Server tự động tạo file `.json` trong thư mục `/problems`.

### 2. Cơ chế hoạt động (AI Inject)
